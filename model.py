...
class MDTA(nn.Module):
    def __init__(self, channels, num_heads):
        ...
        self.q_conv = GCB(5, channels)
        self.k_conv = GCB(3, channels)
        self.v_conv = GCB(1, channels)
...
class Restormer(nn.Module):
    def __init__(self, num_blocks=[4, 6, 6, 8], num_heads=[1, 2, 4, 8], channels=[48, 96, 192, 384], num_refinement=4,
        ...
        self.res1 = DIP(channels[0])
        self.res2 = DIP(channels[1])
        self.res3 = DIP(channels[2])
    ...
    def forward(self, x):
        ...
        res1 = self.res1(out_enc1)
        res2 = self.res2(out_enc2)
        res3 = self.res3(out_enc3)

        out_dec3 = self.decoders[0](self.reduces[0](torch.cat([self.ups[0](out_enc4), res3], dim=1)))
        out_dec2 = self.decoders[1](self.reduces[1](torch.cat([self.ups[1](out_dec3), res2], dim=1)))
        fd = self.decoders[2](torch.cat([self.ups[2](out_dec2), res1], dim=1))
